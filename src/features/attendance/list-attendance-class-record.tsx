...
function ListAttendanceClassRecord({ className, filter }: Props) {
  const { records, isValidating, isLoading } =
    useGetAttendanceSchoolClassRecords(
      {
        classId: filter?.classId,
        date: format(filter?.date || new Date(), 'yyyy-MM-dd'),
      },
      !filter?.classId,
    )
...
export default memo(ListAttendanceClassRecord)

type RecordItemProps = HTMLAttributes<HTMLDivElement> &
  PropsWithChildren & {
    data: AttendanceSchoolClassRecord
  }
            ...
            <div className="sm:space-y-1">
              <div className="space-x-4">
                <span className="text-xl text-brand-secondary sm:text-2xl lg:text-3xl">
                  {`${t('class')} ${data?.school_class.short_title}`}
                </span>
              ...
              <div className="sm:text-md space-x-2 text-sm text-muted-foreground/70 lg:text-lg">
                <span>{`${t('common.classroom_c', { class: data?.school_class.short_title })} - ${t('common.n_students', { number: data?.student_list.length })}`}</span>
              </div>
            </div>
