  ...
  if (usersList) {
    ...
    if (count === 1) {
      return currentUserInList
        ? "You"
        : userArray.find((user) => user.name == usersList[0])?.full_name;
    } else if (count === 2) {
      if (currentUserInList) {
        const otherUser = usersList.find(
          (user, index) => index !== currentUserIndex,
        );
        return `You and ${
          userArray.find((user) => user.name == otherUser)?.full_name ??
          otherUser
        }`;
      } else {
        const user_1 = userArray.find(
          (user) => user.name == usersList[0],
        )?.full_name;
        const user_2 = userArray.find(
          (user) => user.name == usersList[1],
        )?.full_name;
      ...
      if (currentUserInList) {
        ...
        return `You, ${
          userArray.find((user) => user.name == otherUsers[0])?.full_name
        } and ${
          userArray.find((user) => user.name == otherUsers[1])?.full_name
        }`;
      } else {
        const user_1 = userArray.find(
          (user) => user.name == usersList[0],
        ...
        const user_3 = userArray.find(
          (user) => user.name == usersList[2],
        )?.full_name;
      ...
      if (currentUserInList) {
        ...
        return `You, ${
          userArray.find((user) => user.name == otherUsers[0])?.full_name
                ...
                ${
                  userArray.find((user) => user.name == otherUsers[1])
                    ?.full_name
        ...
        return `${
          userArray.find((user) => user.name == usersList[0])?.full_name
                ...
                ${
                  userArray.find((user) => user.name == usersList[2])?.full_name
                } and ${count - 3} other${count - 3 > 1 ? "s" : ""}`;
      }
    }
  }
