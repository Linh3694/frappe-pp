import {
  ...
  MessageBlock,
  MessagesWithDate,
} from "@/types/Messaging/Message";

import { useCallback, useContext, useRef, useState } from "react";
import { Virtuoso } from "react-virtuoso";
import { VirtuosoRefContext } from "@/app/pages/messaging/context/virtuoso-ref-provider";
import {
  ChannelListItem,
  DMChannelListItem,
...
import { ChannelHistoryFirstMessage } from "../empty-state";
import { MessageItem } from "../chat-message/message-item";
import { DateMonthYear } from "@/core/utils/date-conversions";
import {
  DeleteMessageDialog,
  useDeleteMessage,
} from "../chat-message/message-actions/delete-message";
import {
  EditMessageDialog,
  useEditMessage,
} from "../chat-message/message-actions/edit-message";
...
export const ChatHistory = ({
  ...
  const onReplyMessageClick = (messageID: string) => {
    if (virtuosoRef?.current) {
      call
        .get("parent_portal.api.pp_message.get_index_of_message", {
          channel_id: channelData.name,
          message_id: messageID,
        })
        .then((result) => {
          virtuosoRef.current?.scrollToIndex({
            index: parseInt(result.message) ?? "LAST",
            align: "center",
            behavior: "smooth",
          });
        });
...
const Header = ({ context: { channelData } }: any) => (
