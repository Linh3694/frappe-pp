...
} from "./chat-sidebar/sidebar-comp";
      ...
      <SidebarGroupItem gap="2" className={"pl-1.5"}>
        ...
        <Flex width="100%" justify="between" align="center" gap="2">
          <Flex gap="2" align="center">
            <SidebarGroupLabel className="cal-sans">
              Direct Messages
            </SidebarGroupLabel>
          </Flex>
          {!showData &&
            unread_count &&
            unread_count?.total_unread_count_in_dms > 0 && (
              <SidebarBadge>
...
const DirectMessageItemList = ({
  unread_count,
}: {
  unread_count?: UnreadCountData;
}) => {
  const { dm_channels } = useContext(
    ChannelListContext,
  ) as ChannelListContextType;
...
const DirectMessageItem = ({
  ...
  return (
    <SidebarItem to={channel.name} className={"py-0.5"}>
      <SidebarIcon>
        <UserAvatar
      ...
      <Flex justify="between" width="100%">
        <Text
          ...
          weight={unreadCountForChannel ? "bold" : "regular"}
        >
          {channel.peer_user_id !== currentUser
            ? userData?.full_name ?? channel.peer_user_id
...
const ExtraUsersItemList = () => {
  const { extra_users, mutate } = useContext(
    ChannelListContext,
  ) as ChannelListContextType;
  const { call } = useFrappePostCall<{ message: string }>(
    "parent_portal.api.pp_channel.create_direct_message_channel",
  );
