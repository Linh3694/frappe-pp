...
export const useCurrentChannelData = (channelID: string) => {
  const { channels, dm_channels, error, isLoading } = useContext(
    ChannelListContext,
  ) as ChannelListContextType;

  const channel: CurrentChannelData | CurrentChannelDMData | undefined =
    useMemo(() => {
      if (channelID) {
        ...
        const dm_channel = dm_channels?.find(
          (channel) => channel.name === channelID,
        ) as DMChannelListItem;
        if (dm_channel) {
          return {
